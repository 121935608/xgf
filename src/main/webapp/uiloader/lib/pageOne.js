//dataTable init
function _Datatable_InitOne(_dataTable, _columns, _url, lengths, _sortName, _sortOrder) {
	if (typeof (_dataTable) == "undefined") {
		var _sort = typeof(_sortName)=="undefined" ? [] : [_sortName, _sortOrder];
		_dataTable = $('.table-sort-One').dataTable({
			"language" : {
			...
			"fnServerData" : retrieveDataOne			// 源加载数据的表的内容
		});
	} else {
...
// 处理台请求数据
function retrieveDataOne(sSource, data, fnCallback) {
	$.ajax({
