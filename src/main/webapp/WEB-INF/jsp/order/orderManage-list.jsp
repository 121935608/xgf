...
<nav class="breadcrumb"><i class="Hui-iconfont">&#xe67f;</i> 首页 <span class="c-gray en">&gt;</span> 订单管理 <span class="c-gray en">&gt;</span>
    订单管理 <a class="btn btn-success radius r" style="line-height:1.6em;margin-top:3px"
...
//                row.platform;
            ...
            },
            {
                ...
                "mRender": function (data, type, row) {
                    ...
                    if (row.orderStatus == 6 && row.confirmOrder == 0) {
                        toAuditing = "<a title=\"审核\" href=\"javascript:;\" onclick=\"order_auditing('审核','${context_root}/order/toAuditingInfo.action?orderNumber=" + row.orderNumber + "','1000','500')\" class=\"ml-5\" style=\"text-decoration:none\"><span style='color: #0e90d2 '>审核</span></a>";
                    }
                    // 确认出库
                    var toComfirmOrder = "";
    ...
    function query() {
        ...
        pageTable.fnSettings().sAjaxSource = encodeURI("${context_root}/order/findOrderManageList.action?beginTime=" + beginTime + "&endTime=" + endTime + "&payType=" + payType + "&payCode=" + payCode + "&platform=" + platform + "&orderStatus=" + orderStatus + "&orderNumber=" + orderNumber + "&address=" + address + "&supervisorName=" + supervisorName);
        pageTable.fnClearTable(0);
        pageTable.fnDraw();
    }
